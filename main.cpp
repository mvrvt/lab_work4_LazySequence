...
int ReadInt( int min, int max ) {
    ...
    while ( std::cin.fail() || value < min || value > max ) {
        ...
        std::cout << "Ошибка! Число от " << min << " до " << max << ": ";
        std::cin >> value;
    }
    return value;
}
...
int main( int argc, char* argv[] ) {
    ...
    int choice = 0;
    while ( choice != 3 ) {
        std::cout << "\n==== Лабораторная работа №4: Машина состояний (FSM) ====" << std::endl;
        ...
        if ( choice == 1 ) {
            ...
            for ( char c : input ) char_seq->Append(c);

            bool is_valid = email_validator.Process( char_seq );
            std::cout << "-----------------------------------------------------------" << std::endl;
            if ( is_valid ) std::cout << "  РЕЗУЛЬТАТ: Email '" << input << "' ВАЛИДЕН!" << std::endl;
            else std::cout << "  РЕЗУЛЬТАТ: Ошибка! Стоп-состояние: " << email_validator.GetCurrentState() << std::endl;
            std::cout << "-----------------------------------------------------------" << std::endl;
            delete char_seq;
        } else if ( choice == 2 ) {
            std::cout << "\n===========================================================" << std::endl;
